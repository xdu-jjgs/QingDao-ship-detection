...
class VideoCapture:
    def __init__(self, src_rtsp_url):
        ...
        self.thread = threading.Thread(
            target=self._reader, name='VideoCapture')
        self.thread.start()
            ...
            try:
                ...
                if not self.ret:
                    ...
                    if not self.stop_event.is_set():
                        ...
                        self.reconnect_time = min(
                            self.reconnect_time * 2, max_reconnect_time)  # 指数回退，最大值为 5 分钟
                else:
                    self.reconnect_time = base_reconnect_time
            except Exception:
...
class CameraPos:
    def __init__(self, video_id):
        ...
        self.thread = threading.Thread(
            target=self.request_camera_ptz, name='CamearPos')
        self.thread.start()
    ...
    def get_camera_position(self):
        return self.tilt, self.zoom, self.pan
        ...
        if self.flag == False or self.target_tbox_id is None:
            ...
            try:
                ...
                if self.flag_cnt == 0:
                    ...
                    matching_box = next(
                        (box for box in tboxes_now if box.id == self.target_tbox_id), None)
                    target_box = [matching_box.id, matching_box.x0,
                                  matching_box.y0, matching_box.x1, matching_box.y1]
...
class CameraMoveStatus:
    ...
    def move_finish(self):
        if self.camera_moved_finished == True:
            return
...
def match_shiptext2ship(ship_tboxes, text_bboxes, ocr_texts):
    ...
    for tbox in ship_tboxes:
        ...
        ship_dict[tbox.id] = {'bbox': bbox, 'cls': tbox.cls,
                              'speed': tbox.speed, 'text_bbox_words': text_bbox_words}
        # tbox 添加 text 字段
        tbox['text_word'] = text_bbox_words
