...
class LWIRShipDetector:
    ...
    def __call__(self, frame: np.ndarray) -> List[dict]:
        ...
        for xyxysc in pred:
            ...
            score = xyxysc[4]
            cls = int(xyxysc[5])
            bbox = EasyDict({'x0': x0, 'y0': y0, 'x1': x1, 'y1': y1, 'w': x1 - x0, 'h': y1 - y0, 'prob': score, 'cls': cls, 'lbl': 'ship'})
            bboxes.append(bbox)
...
class ShipDetector:
    ...
    def __call__(self, frame: np.ndarray) -> List[dict]:
        ...
        for xyxysc in pred:
            ...
            score = xyxysc[4]
            cls = int(xyxysc[5])
            bbox = EasyDict({'x0': x0, 'y0': y0, 'x1': x1, 'y1': y1, 'w': x1 - x0, 'h': y1 - y0, 'prob': score, 'cls': cls, 'lbl': cls2lbl[int(cls)]})
            bboxes.append(bbox)
...
class ShipTracker:
    ...
    def reset(self):
            ...
            tilt, zoom, pan = self.camera_pos.get_camera_position()
            # 1/1.8"传感器 分辨率 2560×1440
            self.model = ByteTrack(
                conf_thresh=0.2,
                sensor_w=6.83,
                sensor_h=3.89,
                camera_install_height=104,
                image_w=2560,
                image_h=1440,
                frame_rate=25,
                tilt=tilt,
    ...
    def __call__(self, frame: np.ndarray, bboxes: List[dict]) -> List[dict]:
        ...
        tboxes = []
        tilt, zoom, pan = self.camera_pos.get_camera_position()
        self.model.update_zoom_tilt_pan(tilt, zoom, pan)
        ...
        for trk in trks:
            ...
            distance = self.model.get_distance[trk.track_id]
            tbox = EasyDict({
                'x0': x0,
                'y0': y0,
                'w': x1 - x0,
...
class TextDetector:
    ...
    def __call__(self, frame: np.ndarray) -> List[dict]:
        ...
        for xyxysc in pred:
            ...
            bbox = EasyDict({'x0': x0, 'y0': y0, 'w': x1 - x0, 'h': y1 - y0, 'x1': x1, 'y1': y1})
            bboxes.append(bbox)
        return bboxes
