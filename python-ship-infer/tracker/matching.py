...
import tracker.kalman_filter as kalman_filter
import math 
...
def linear_assignment(cost_matrix, thresh):
    ...
    unmatched_a = list(set(range(cost_matrix.shape[0])) - set(list(x))) + delete_x
    unmatched_b = list(set(range(cost_matrix.shape[1])) - set(list(y))) + delete_y
...
def ious(atlbrs, btlbrs):
    ...
    """
    ious = np.zeros((len(atlbrs), len(btlbrs)), dtype=np.float16)
    if ious.size == 0:
        return ious
...
def embedding_distance(tracks, detections, metric='cosine'):
    ...
    if cost_matrix.size == 0:
        return cost_matrix
    det_features = np.asarray([track.features[-1] for track in detections], dtype=np.float16)
    track_features = np.asarray([track.features[-1] for track in tracks], dtype=np.float16)
    if metric == 'euclidean':
