import numpy as np
import cv2
from depth_anything_v2.trt_models.utils_function import load_image
import platform
...
class Dpt:
    ...
    def __init__(self, trt_path, gpu_id, img_size) -> None:
        self.reshape_size = [img_size, img_size]
...
if __name__ == "__main__":
    ...
    # 打开视频文件
    video_path = "/media/F/yangzewei/QingDao_projrect/QingDao-ship-detection/videos/shift2center.mp4" 
    # 导入trt权重
    trt_path = '/media/F/yangzewei/QingDao_projrect/QingDao-ship-detection/python-ship-infer/depth_anything_v2_vits.trt'
    model = Dpt(trt_path, gpu_id=1, img_size=518)
    ...
    while True:
        ...
        # 推理
        t1 = time.time()
        frame, shape_info = model.preprocess(frame)
        depth_res = model.inference(frame)
        depth_img = model.postprocess(shape_info, depth_res)
        t2 = time.time()
        print(t2-t1)
        cv2.imwrite(f'python-ship-infer/depth_anything_v2/vis_depth/{cnt}.jpg', depth_img)
        cnt += 1